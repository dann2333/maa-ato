...
### E-7 集成检验样本数太小，结论随种子翻转 ✅ 已修（默认 n=40）

**症状**：同一关卡、同一计划、只换随机种子：

| n | seed=7 | seed=11 | seed=23 |
|---|---|---|---|
| 10 | fragile (80%) | **robust (100%)** | fragile (80%) |
| 40 | robust (90%) | robust (95%) | robust (95%) |

0.85 的阈值正好卡在 n=10 能取到的两个相邻样本率（0.8 与 0.9）之间，
于是"这个计划稳不稳"变成了抛硬币——**比没有结论更糟，因为它带着一个名字**。

**修法**：`DEFAULT_ENSEMBLE_N = 40`（原为 8）。代价是单次检验 3 秒 → 12 秒。
n=40 时距阈值仍只有约一个标准误，所以**结论必须连同 n 一起报告**，
单次运行不算定论。

**顺带修掉的**：`sample_calibration` 里防止钳位上下界互穿的守卫假定
`aspd_min/aspd_max` 一定在 `CALIBRATION_BANDS` 里，改成按现值兜底。

### E-8 `ROBUST` 只看通关率，不看余量 ⬜ 未修（设计问题，不是 bug）

n=40 下 `main_01-07` 被判 `robust`，而它在最差的一次扰动里**掉光了全部 11 点血**，
40 次里有 4 次没通关。阈值 `ROBUST_CLEAR_RATE = 0.85` 的注释是站得住的
（随机化区间故意比现实宽），但"robust"这个词不应该和"会失败"相容。

**应该怎么改**：判据里加余量项（掉血头寸），而不是只看通关率。
**为什么先不改**：这是保真度层的语义变更，不是算错了；它会改变所有已记录的裁决，
应该和标定协议（§D）一起重新设计，而不是顺手调一个阈值。
...
| 量 | 修复前 | 修复后 |
|---|---|---|
| 规划耗时（120 rollout） | 51.7 s | 31.9 s |
| 规划结果 | CLEARED 41/41，漏 0 | CLEARED 41/41，漏 0 |
| 首次部署 | t=27.6 s（DP≥14） | t=0.0 s（DP≥10） |

首次部署时刻的变化正是修复的意义：原来的计划是对着一个走慢了的钟排的。

### 集成检验（n=40, seeds 7/11/23）

| 量 | 数值 |
|---|---|
| 通关率 | 90% / 95% / 95% |
| 击杀时刻 L1 中位 | 3.84–4.02 s |
| 击杀时刻 L1 最差 | 12.76–12.88 s |
| 扰动下掉血 均值 / 最差 | 2.4–3.4 / **11（即失败）** |
| 裁决 | robust（但见 E-8） |
| 信任等级 | **UNTRUSTED** |
| `allows_training` | **False** |

**不要拿这一段跟"修复前"比。** 修复前记录的是 `robust / 掉血最差 1`，
那是 n=10 的采样假象：在同样的 n=10 下换个种子就能得到 fragile（见 E-7）。
两列不可比，这里只记录 HEAD 的真实数字。

**信任等级从 IMITATION 掉到 UNTRUSTED，这是台账在正常工作。**
承认了攻速钳位与两个定时语义是未知量之后，这个计划对**我们自己的无知**并不稳健——
它在 40 次扰动里失败 4 次。台账按"见过的最坏情况"封顶信任，所以它降级了。
这条信息比一个漂亮的 robust 有用得多。

而且无论如何：以上全部是 sim-vs-sim 证据，**再漂亮也不能晋级到 `TRAINING`**——
两次模拟跑得像，跟真实游戏无关。
